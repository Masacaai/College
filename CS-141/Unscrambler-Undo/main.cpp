...
public:
    ...
    // Mutator methods
    void setTheBoard(string theBoard){ Board::theBoard = theBoard;}
    void setCompletedBoard(string completedBoard){Board::completedBoard = completedBoard;}
    void setboardWords(vector <string> boardWords){Board::boardWords = boardWords;}
    void setBoardCharsPerSide(int boardCharsPerSide){Board::boardCharsPerSide = boardCharsPerSide;}
    void setBoardSize(){boardSize = boardCharsPerSide*boardCharsPerSide;}

    // Accessor Methods
    string getTheBoard(){return theBoard;}
    string getCompletedBoard(){return completedBoard;}
    vector <string> getboardWords(){return boardWords;}
    ...
    void handleRotation();
    void scrambleBoard();
    void setUpBoards(vector<string> dictionaryWords);
...
private:
    ...
    long unsigned int boardSize;    // Total length of chars on board
};
        ...
        vector<string> dictionaryWords)	    // Vector of dictionary words
{
    ...
    while( allWordsPlusSpaces.length() < boardSize) {
        ...
        if( (randomWord.length() + allWordsPlusSpaces.length()) > boardSize) {
            continue;
        }
...
void Board::handleRotation(){
    ...
    while( true) {
        ...
        if( !( (rowOrCol >= '0' && rowOrCol <= '0'+boardCharsPerSide - 1) ||   // Row is within valid row range
            ...
            // It is not within the valid range
        ...
        vector<string> dictionaryWords) // List of all dictionary words, for random selections
{
    ...
    setBoardCharsPerSide(boardCharsPerSide);
    setBoardSize();

    // Blank out all initial boards, in case this is not the first time doing setup.
    setTheBoard("");               // Playing board
    setCompletedBoard("");    // Completed board, used to check for win
...
void displayList( Node *pTemp) {
    ...
    cout << "   List is: ";
    while(pTemp != NULL){
        cout << pTemp->moveNumber << ":" << pTemp->board.getTheBoard();
        if (pTemp->pNext != NULL){
            cout << " -> ";
        }
        pTemp = pTemp->pNext;
...
void deleteFrontNodeFromList( Node *&pHead, Board & board, int & moveNumber ) {
    ...
    if (pHead->pNext == NULL){
        ...
        // . . .
        board.setTheBoard(pHead->board.getTheBoard());
...
void prependNodeToList( Node *&pHead, Board board, int moveNumber) {
...
int main()
{
    ...
    gameBoard.setUpBoards(dictionaryWords);
    // Add scrambled board as a node in linked list
    prependNodeToList(pHead,gameBoard,moveNumber);
    ...
    while(true) {
        ...
        switch( userInput) {
            ...
            case 'G': {
                ...
                // Reset linked list by deleting all existing nodes
                while( pHead != NULL){
                    Node *pTemp = pHead;
                    pHead = pHead->pNext;
                    delete pTemp;
                }
                moveNumber = 1; // Reset moveNumber to 1
            ...
            case 'U': {
                deleteFrontNodeFromList(pHead, gameBoard, moveNumber);
                break;
            }
            default: {
        ...
        // Check for a win
        if( gameBoard.getTheBoard() == gameBoard.getCompletedBoard()) {
            gameBoard.displayBoard();
