/*
    ...
    Program Overview:
    This file contains the functions from the hashmap
    class used to implement Huffman file compression.
*/
...
void hashmap::put(int key, int value) {
    key_val_pair* cur = buckets[hashFunction(key) % 10];
    ...
    // Create node and add it to the hashmap
    ...
    if (prev)
        ...
        buckets[hashFunction(key) % 10] = node;
    nElems++;
}
...
int hashmap::get(int key) const {
    ...
    __throw_runtime_error("Key not found!");
    return 0;
}

//
// This function checks if the key is already i1n the map.
//
